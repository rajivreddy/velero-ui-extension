...
export default {
  ...
  computed: {
    ...
    snapshotType() {
      return this.value.spec?.snapshotType || '-';
    },

    sourceNamespace() {
      return this.value.spec?.sourceNamespace || '-';
    },

    sourcePVC() {
      return this.value.spec?.sourcePVC || '-';
    },

    csiSnapshot() {
      return this.value.spec?.csiSnapshot || null;
    },
    ...
    snapshotID() {
      return this.value.status?.snapshotID || '-';
    },
  ...
  >
    ...
    >
      ...
      <div class="detail-cards">
        ...
        <div class="detail-card">
          <h4>Snapshot Type</h4>
          <span>{{ snapshotType }}</span>
        </div>

        <div class="detail-card">
          <h4>Source PVC</h4>
          <span>{{ sourceNamespace }}/{{ sourcePVC }}</span>
        </div>
      ...
      <div class="detail-cards">
        ...
        <div v-if="path !== '-'" class="detail-card wide">
          <h4>Path</h4>
          <code>{{ path }}</code>
        </div>
      </div>

      <div v-if="csiSnapshot" class="row mb-20 mt-20">
        <div class="col span-12">
          <h3>CSI Snapshot</h3>
        </div>
      </div>

      <div v-if="csiSnapshot" class="detail-cards">
        <div class="detail-card">
          <h4>Snapshot Class</h4>
          <span>{{ csiSnapshot.snapshotClass || '-' }}</span>
        </div>

        <div class="detail-card">
          <h4>Storage Class</h4>
          <span>{{ csiSnapshot.storageClass || '-' }}</span>
        </div>

        <div class="detail-card wide">
          <h4>Volume Snapshot</h4>
          <span>{{ csiSnapshot.volumeSnapshot || '-' }}</span>
        </div>
      </div>
    </Tab>
  </ResourceTabs>
